...
create table if not exists public.materials (
  ...
  material_type text not null check (
    material_type in ('PLA','PETG','ABS','TPU','ASA','Nylon','Other')
  ),
  brand         text,
  color         text,
  price_per_kg  numeric(10,4) not null default 0 check (price_per_kg >= 0),
...
create index if not exists materials_user_active_idx on public.materials(user_id, is_active);
